...
#define TSMessageViewImageOffset 15.0
...
canBeDismissedByUser:(BOOL)dismissingEnabled
{
    ...
    if ((self = [self init]))
    {
        ...
        if (image) self.textSpaceLeft += image.size.width + 3 * TSMessageViewImageOffset;
        ...
        if (image)
        {
            ...
            self.iconImageView.frame = CGRectMake(TSMessageViewImageOffset,
                                                  3,
                                                  image.size.width*2,
                                                  image.size.height*2);
